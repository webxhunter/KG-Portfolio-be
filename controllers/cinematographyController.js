...
export const upsertCinematographyVideo = async (req, res) => {
  try {
    ...
    const [rows] = await pool.query(
      'SELECT * FROM cinematography_videos WHERE category = ?', 
      [category]
    );
    ...
    if (rows.length > 0) {
      ...
      await pool.query(
        'UPDATE cinematography_videos SET image_url = ?, location = ? WHERE category = ?',
      ...
      await pool.query(
        'INSERT INTO cinematography_videos (category, image_url, location) VALUES (?, ?, ?)',
...
export const addCinematographyVideo = async (req, res) => {
  try {
    ...
    await pool.query(
      'INSERT INTO cinematography_gallery_video (category, image_url, location) VALUES (?, ?, ?)',
      [category, image, location]
    );
...
export const updateCinematographyGalleryVideo = async (req, res) => {
  try {
    ...
    const [rows] = await pool.query(
      'SELECT * FROM cinematography_gallery_video WHERE id = ?', 
      [id]
    );
    ...
    if (newImage && oldImage) {
      const oldPath = path.join(process.cwd(), 'public/uploads', oldImage);
      if (fs.existsSync(oldPath)) {
        fs.unlinkSync(oldPath);
      }
    }
    ...
    await pool.query(
      'UPDATE cinematography_gallery_video SET category=?, location=?, image_url=? WHERE id=?',
      [
...
export const deleteCinematographyGalleryVideo = async (req, res) => {
  try {
    ...
    // ✅ SAFE DELETE
    if (rows[0].image_url) {
      const imagePath = path.join(process.cwd(), 'public/uploads', rows[0].image_url);
    ...
    await pool.query(
      'DELETE FROM cinematography_gallery_video WHERE id = ?', 
      [id]
    );
