...
export const updateService = async (req, res) => {
  ...
  try {
    await pool.query(
      'UPDATE services SET media_type=?, media_url=? WHERE name=?',
      ['image', mediaUrl, name] // ✅ always image now
    );

    res.json({ success: true, mediaType: 'image', mediaUrl });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
};
    ...
    await pool.query(
      `INSERT INTO service_media (service_name, position, media_type, file_path)
       ...
       ON DUPLICATE KEY UPDATE 
         ...
         file_path = VALUES(file_path),
         updated_at = NOW()`,
      [serviceName, position, 'image', filePath] // ✅ force image
    );
